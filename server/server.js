...
//     if (ext !== '.pdf' || ext !== '.doc') {
//       return cb(res.status(400).end('only pdf, doc is allowed'), false);
//     }
...
app.post('/api/careers', multer, (req, res, next) => {
  const url = req.protocol + '://' + req.get('host');
  req.body.careers = JSON.parse(req.body.careers);
  const careers = new Careers({
    fullName: req.body.careers.fullName,
    email: req.body.careers.email,
    role: req.body.careers.role,
    fileUrl: url + '/images/' + req.file.filename,
    whyJoin: req.body.careers.whyJoin,
  });
  careers
    .save()
    .then(() => {
      res.json({
        status: true,
        response: 'Success',
        message: 'Your detials received successfully.',
      });
    })
    .catch((error) => {
      res.status(400).json({
...
app.post('/api/subscribe', (req, res) => {
  ...
  const options = {
    url: 'https://us6.api.mailchimp.com/3.0/lists/93e7386ba6',
    method: 'POST',
    headers: {
      Authorization: 'auth ba26244b6e5ebd16074363817880580f-us6',
    },
    body: mcDataPost,
  };
